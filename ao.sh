...
# checking whether test.c exists
	...
	SETUP=$(cat "$TEST" | grep -o '\([[:space:]]*int[[:space:]]\)\?[[:space:]]*setup[[:space:]]*(')
	AT_CASE_FAIL=$(cat "$TEST" | grep -o '[[:space:]]*void[[:space:]]\{1,\}at_case_fail[[:space:]]*(')
	AT_CASE_EXIT=$(cat "$TEST" | grep -o '[[:space:]]*void[[:space:]]\{1,\}at_case_exit[[:space:]]*(')
	AO_STATS=$(cat "$TEST" | grep -o '\([[:space:]]*int[[:space:]]\)\?[[:space:]]*ao_stats[[:space:]]*(')
	AORUNNER=""
