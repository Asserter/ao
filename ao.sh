...
VERSION="Assert Operator (debug-0.1.0)"
...
# checking whether test.c exists
	...
	AT_CASE_FAIL=$(cat "$TEST" | grep -o "at_case_fail")
	AT_CASE_EXIT=$(cat "$TEST" | grep -o "at_case_exit")
#	AO_STATS
	AORUNNER=""
...
# handling test.data if exists
	if [ -f "$DATA" ]; then
		...
		if [ -n "$AORUNNER" ]; then
			...
			printf "\n\t\t(size_t(*)(const struct ao *))%s," $AO_STATS >> "$MAIN"
			printf "\n\t\t%s,"  $AOINC >> "$MAIN"
			printf "\n\t\t%s"   $AOMODE >> "$MAIN"
